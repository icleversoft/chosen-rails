class AbstractChosen
  ...
  custom_label: (html) ->
    _source = html.replace(/&amp;/g, '&').replace(/&nbsp;/,'<span>&nbsp;</span>')
    return _source unless _source.match(/\{fa-.[^}]+}/g)
    for _icon in _source.match(/\{fa-.[^}]+}/g)
      _source = _source.replace(_icon, '<i class="fa fa-icon '+_icon.match(/fa.[^}]+/)[0]+'"/>')
    _source
  ...
  result_add_option: (option) ->
    ...
    option_el.innerHTML = this.custom_label(option.search_text)
